...
test.describe('Room Management', () => {
  ...
  test('should create a new room successfully', async () => {
    const roomData = createRoomData({ type: 'Double', price: 150 });
  ...
  test('should show the new room in the list after creation', async () => {
    const roomData = createRoomData({ type: 'Single', price: 100 });

    await roomsPage.createRoom(roomData);

    await expect(
      roomsPage.page.locator(`.room-listing .row:has-text("${roomData.roomNumber}")`)
    ).toBeVisible();
  });

  test('should display room details after creation', async () => {
    const roomData = createRoomData({ type: 'Suite', price: 300, accessible: true });

    await roomsPage.createRoom(roomData);

    const roomRow = roomsPage.page.locator(`.room-listing .row:has-text("${roomData.roomNumber}")`);
